...
if __name__ == '__main__':
    ...
    for entry in internetConnection:
        if (entry['state'] == 'LISTEN' or entry['state'] == 'ESTABLISHED') and (entry["foreignAddress"].split(":")[0].split(".")[0] != '127' and (entry["localAddress"].split(":")[-1] == "*" or int(entry["localAddress"].split(":")[-1]) > 1000)) and (entry["localAddress"].split(":")[0].split(".")[0] != '127' and (entry["foreignAddress"].split(":")[-1] == "*" or int(entry["foreignAddress"].split(":")[-1]) > 1000)):
